...
*Примітка по виконанню:* `src/flight/setpoints.py` замінив п'ять атрибутів контролера і `__set_throttle_base`. Клампінг реплікує рівно те, що контролер реально enforced: лише верхню межу газу (`>= THROTTLE['max']`, колишній хардкод 1800); нижню межу `THROTTLE['min']` та вікно `PWM_LIMITS` для roll/pitch/yaw, як і раніше, тримають їхні власники (`AltitudeController`/`PositionController`, np.clip власних виходів) — нових клампів не додано. Сеттери нормалізують PWM до вбудованого `int` — no-op для всіх живих шляхів (numpy-інти PositionController пакуються pymavlink'ом у uint16 через struct байт-у-байт однаково; анотації `AttitudeSetpoints` стали чесними в точці споживання). Поглинуто з рев'ю кроку 5: (а) `InterceptGuidance.update()` повертає frozen `InterceptResult` (`setpoints`/`active`/`reenable_stabilization`) — оркестратор більше не виводить активність із власного `target is not None`, а re-enable стабілізації перестав бути side effect'ом: контролер виконує його сам (та сама умова `not is_connected`, той самий момент — синхронно в тій же ітерації, до запуску StabilizationBehavior; між ними лише чисте присвоєння RC-стану), залежність InterceptGuidance від StabilizerManager прибрано; (б) видалено тест-ергономічний дефолт `intercept_active=False` у `StabilizationBehavior.update` — gate-вхід тепер явний у всіх викликачів.
...
*Примітка по виконанню:* `src/app.py:build_controller()` конструює граф у ТОМУ Ж порядку, що й старий `DroneController.__init__` (логер контролера → MAVLink+connect → сенсор+start → StabilizerManager з ін'єктованим SkyAnchorClient → battery → PID-контролери → RCSetpoints → detection → behaviors → SignalHandler → CommandHandler з ін'єктованим TelnetServer 0.0.0.0:2323), тому стартові лог-рядки/потоки/сокети йдуть у незмінному порядку; `run_controller.py` викликає `build_controller()` (модульний старт збережено, шим `xbee_process_com.py` не чіпали). `DroneController.__init__` тонкий: лише зберігає колаборатори, реєструє команди, стартує telnet-обробку та ARM_IN-таймер; параметри обов'язкові keyword-only БЕЗ None-fallback'ів — grep підтвердив, що ніхто, крім run_controller, контролер не конструює (`examples/` взагалі не імпортують `src`; PositionController контролеру більше не потрібен — ним володіє StabilizationBehavior). Конфіг: `src/config/objects.py` — frozen `PIDGains`/`AltitudeConfig`/`PositionConfig`, значення читаються фабриками `from_dicts()` з існуючих dict'ів (жодного числа руками); контролери отримали optional `config=None` → `from_dicts()` (ті самі значення байт-у-байт, тести/характеризація без змін), `__enable_stabilization` відновлює ki/kd з ін'єктованого config замість глобальних `POSITION_PID_X/Y`; читання модульних dict'ів у гарячому `update()` (THROTTLE/CONTROL/DEBUG, COORDINATE_SYSTEM/POSITION_CONTROL/PWM_LIMITS/ALTITUDE_COMPENSATION, включно з `'key' in dict`-перевірками) свідомо НЕ мігровано — мінімальний дизайн без зміни поведінки. `USE_GAZEBO`: `detection_config` тепер імпортує з `controller_config` (одне читання env у `src/`; окремий діагностичний скрипт `check_gazebo_altitude.py` читає env напряму з ІНШОЮ семантикою truthiness — не чіпали свідомо). Перенесені пункти: DetectionServer володіє `INTERCEPT_*` як конструкторними дефолтами (контролер кличе `get_active_target()` без аргументів; явні параметри лишились і перекривають); tripwire у `StabilizerManager.poll_new()` — warn-once на `matches_percent > 100` із `navigation=False` (читання не відкидається); `attempt_connection()` видалено (нуль викликачів); `is_connected` ЛИШИВ one-way latch замість прямої делегації — делегація втратила б одноразовий лог смерті з'єднання і зламала б закріплену тестами незворотність; `RCSetpoints(throttle_max=THROTTLE['max'])` — стеля газу як параметр, app.py передає явно; `__current_altitude`/`__is_up` (та `__mode` PositionController) мігровано у інстанс-атрибути; тест імутабельності перейшов на `dataclasses.FrozenInstanceError`. Нові тести: `test_app.py` (форма графа з мокнутими межами MAVLink/Telnet/CSV/Signal), `test_controller.py` (тонка конструкція без I/O, ARM_IN-таймер, авто-setHeight(5) при arm, споживання `reenable_stabilization` — закрито прогалину рев'ю кроку 6), `test_config_objects.py` (рівність конфіг-об'єктів із dict'ами полем-у-поле). 395 тестів зелені, golden JSON не перегенеровано.
