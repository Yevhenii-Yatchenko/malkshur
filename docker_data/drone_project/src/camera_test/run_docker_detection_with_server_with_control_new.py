...
DOCKER_RUN_SCRIPT = "../../jetson-inference/run-rtx4090.sh"
DOCKER_CONTAINER_NAME = "jetson-rtx4090"
DOCKER_PASSWORD = ""
...
def get_jetson_inference_dir(script_path: str) -> str:
    ...
    # script_path is relative (e.g., "../jetson-inference/run-rtx4090.sh")
    # We just need the directory containing the script
...
class DockerDetectionRunner:
    ...
    def stop(self, container_name: str = None) -> None:
        """Stop the logging thread and Docker container."""
        ...
        try:
            ...
            print(f"[INFO] Docker container {container_name} stopped")
        except Exception as e:
...
def run_control_demo() -> None:
    ...
    def cleanup_handler(_sig, _frame):
    ...
    import subprocess
    try:
        print("\n[INFO] Checking for existing Docker container...")
        result = subprocess.run(['docker', 'ps', '-a', '--filter', f'name={DOCKER_CONTAINER_NAME}', '--format', '{{.Names}}'],
                              capture_output=True, text=True, timeout=5)
        if DOCKER_CONTAINER_NAME in result.stdout:
            print(f"[INFO] Found existing container '{DOCKER_CONTAINER_NAME}', stopping it...")
            subprocess.run(['docker', 'stop', DOCKER_CONTAINER_NAME], capture_output=True, timeout=10)
            print("[INFO] Existing container stopped")
    except Exception as e:
        print(f"[WARNING] Could not clean up existing container: {e}")
